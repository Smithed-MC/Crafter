...
function smithed:crafter/block/table/crafting/input/read_barrel
data modify storage smd:crafter root.temp.item set from block ~ ~ ~ Items[{Slot:16b}]
data remove block ~ ~ ~ Items[{Slot:16b}]

execute as @p[distance=..12,tag=smd.inside_crafter,tag=smd.shift_clicked] at @s run function smithed:crafter/block/table/crafting/output/shift_click/spawn_extra_items
function smithed:crafter/block/table/crafting/input/read_barrel
